...
parser.add_argument('--video_path', type=str, required=True)
parser.add_argument('--video_id', type=str, required=True)
parser.add_argument('--video_ide', type=str, required=True)
...
start_num = int(args.video_id)
end_num = int(args.video_ide)

trackers.basetrack.BaseTrack.reset_id()

# Generate a list of numbers between start_num and end_num
number_list = [str(num).zfill(3) for num in range(start_num, end_num + 1)]

for video_id in number_list:
    results = model.track(source=f"{video_path}/{video_id}.mp4", 
                                        ...
                                        save_frames=True, 
                                        save=True, 
                                        project=f'./results_final/{video_id}')
    ...
    text_files_path = f'./results_final/{video_id}/track/labels'
